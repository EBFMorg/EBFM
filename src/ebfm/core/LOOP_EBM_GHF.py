...
def cache_is_valid(OUT):
    """
    True if OUT["ghf_k"]/["ghf_cond"]/["hcap_sub"] match a fresh call to conductance(OUT).
    ...
    """
    GHF_k, GHF_C, hcap_sub = conductance(OUT)
    return (
        np.array_equal(GHF_k, OUT["ghf_k"])
        and np.array_equal(GHF_C, OUT["ghf_cond"])
        and np.array_equal(hcap_sub, OUT["hcap_sub"])
    )
...
def main(Tsurf, OUT, cond, GHF_k, GHF_C):
    ...
    """
    assert cache_is_valid(OUT), "OUT['ghf_k']/['ghf_cond']/['hcap_sub'] are stale"
