...
class ExchangeKeySet:
    """
    The keys communicated in one exchange with a component: the source keys sent by EBFM and the target keys
    received by EBFM.
    ...
    source_keys: AbstractSet[str] = frozenset()  # keys sent by EBFM, i.e. the keys of data_to_exchange
    target_keys: AbstractSet[str] = frozenset()  # keys received by EBFM, i.e. the keys given by target_keys
...
class Component(ABC):
    ...
    """
    Key sets this component accepts in an exchange, i.e. which combinations of source and target keys a caller
    of exchange may ask for. Subclasses have to declare at least one key set.
    """
    ...
    def _put_if_coupled(
        ...
        @note Only call this for the source keys of the requested key set. The caller of exchange is required
              to provide the data for those keys, which exchange checks before anything is communicated.
    ...
    def _get_if_coupled(
        ...
        @note Only call this for the target keys of the exchange that is currently performed. Which keys those
              are is determined by the key set that the caller of exchange requested.
    ...
    def _map_mass_flux_from_ebfm(self, mwe_per_timestep: np.ndarray) -> np.ndarray:
        """
    ...
    def _all_target_keys(self) -> set[str]:
        """
        Get all target keys of this component, i.e. the names of its coupled TARGET fields.

        These are the target keys a caller of exchange requests by default.
    ...
    def _requested_key_set(
        self, data_to_exchange: Mapping[str, np.ndarray], target_keys: Collection[str] | None
    ) -> ExchangeKeySet:
        """
        ...
        @param[in] data_to_exchange Mapping of field names to data to be sent, as given to exchange. Its keys are
                                    the source keys.
        @param[in] target_keys collection of field names to be received, as given to exchange. None is replaced
                               by all target keys of this component (see _all_target_keys).

        @returns the requested key set
        """
        requested_key_set = ExchangeKeySet(
            source_keys=frozenset(data_to_exchange),
            target_keys=frozenset(self._all_target_keys() if target_keys is None else target_keys),
        )
    ...
    def _unsupported_exchange_message(self, requested_key_set: ExchangeKeySet) -> str:
        ...
        source_keys, target_keys = requested_key_set.source_keys, requested_key_set.target_keys

        lines = [
            f"Component '{self.name}' cannot exchange source keys {_format_keys(source_keys)} "
            f"together with target keys {_format_keys(target_keys)}.",
            "The source keys are the keys of data_to_exchange, the target keys are given by target_keys, which "
            "defaults to all target keys of this component.",
        ...
        for accepted_key_set in self.accepted_exchange_key_sets:
            problems = []
            if missing := accepted_key_set.source_keys - source_keys:
                problems.append(f"missing source keys: {_format_keys(missing)}")
            if unexpected := source_keys - accepted_key_set.source_keys:
                problems.append(f"unexpected source keys: {_format_keys(unexpected)}")
            if missing := accepted_key_set.target_keys - target_keys:
                problems.append(f"missing target keys: {_format_keys(missing)}")
            if unexpected := target_keys - accepted_key_set.target_keys:
                problems.append(f"unexpected target keys: {_format_keys(unexpected)}")
            lines.append(
                f"  '{accepted_key_set.name}': source keys {_format_keys(accepted_key_set.source_keys)}, "
                f"target keys {_format_keys(accepted_key_set.target_keys)}"
            )
            if problems:
    ...
    def exchange(
        ...
        The keys of `data_to_exchange` define what is sent (source keys), `target_keys` what is received (target
        keys). Both are unordered and together they have to be one of the key sets this component accepts (see
        ...
        @param[in] target_keys optional collection of field names to be received. Defaults to all target keys
                               of this component, i.e. the names of all its coupled TARGET fields.
        ...
        @raises ValueError if the keys of data_to_exchange and target_keys are not a key set this component
                accepts
        """
        requested_key_set = self._requested_key_set(data_to_exchange, target_keys)
    ...
    def _exchange(
        ...
        Only put the source keys and only get the target keys of the requested key set. A component that
        accepts more than one key set therefore branches on requested_key_set.
