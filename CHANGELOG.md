...
* The `icon_land` component also receives the surface energy balance results of ICON-Land's glacier tile, averaged over the EBFM time step: `t_srf` (K), `melt` and `evapotrans` (converted to m w.e. per time step, evapotrans negative upward). When received, they drive the snow/firn model and the mass balance instead of EBFM's own energy balance (`LOOP_EBM_icon_land`): `Tsurf`, `melt` and the `moist_*` terms are taken from ICON-Land (evapotranspiration partitioned by the surface temperature as before). EBFM's own energy balance is still computed as a diagnostic and kept as `ebm_Tsurf`, `ebm_melt`, `ebm_Emelt` (written to the output) and `ebm_moist_*`. https://github.com/EBFMorg/EBFM/pull/178
